import { TalentData } from "../../TalentContext";
import { talentText, requireAll } from "../../utils";

const backgrounds = requireAll(
  require.context("../../assets/tree-backgrounds/empire"),
);
const icons = requireAll(require.context("../../assets/icons/empire"));

export const data: TalentData = {
  "Free Company": {
    name: "Free Company",
    background: backgrounds["militia"],
    icon: icons["archetype_dps"],
    talents: {
      "Motivated": {
        name: "Motivated",
        pos: "a2",
        icon: icons["rrtac_motivated"],
        maxRank: 1,
        reqPoints: 5,
        type: "renown tactic",
        description: talentText`You gain 5 adrenaline every second while in combat.`,
      },
      "Strength in Numbers": {
        name: "Strength in Numbers",
        pos: "b2",
        icon: icons["rrtac_strengthinnumbers"],
        maxRank: 1,
        reqPoints: 5,
        type: "renown tactic",
        description: talentText`Whenever you use a basic attack, all groupmates within 100 feet that also have this tactic will gain a stack. Max 5 stacks, each stack grants 2 adrenaline per second for a duration of 5 seconds.`,
      },
      "Uncontrolled": {
        name: "Uncontrolled",
        pos: "a4",
        icon: icons["rrtac_uncontrolled"],
        maxRank: 1,
        reqPoints: 15,
        type: "renown tactic",
        description: talentText`Whenever Unstoppable, Inevitable, or Immovable is applied to you, you will gain 20 adrenaline per second over 5 seconds.`,
      },
      "Stay in Formation": {
        name: "Stay in Formation",
        pos: "b4",
        icon: icons["rrtac_stayinformation"],
        maxRank: 1,
        reqPoints: 15,
        type: "renown tactic",
        description: talentText`Whenever you are attacked by an opponent you have Cover from, you will gain 10 adrenaline per second for 3 seconds.`,
      },
      "Gotta Go Fast": {
        name: "Gotta Go Fast",
        pos: "b6",
        icon: icons["rrtac_gottagofast"],
        maxRank: 1,
        reqPoints: 25,
        type: "renown tactic",
        description: talentText`You gain 1 stack of Revelry every second while moving.\n\nRevelry: 1 adrenaline per second. Stacks 15 times and lasts 3 seconds.`,
      },
      "Festive Chaos": {
        name: "Festivity",
        pos: "a6",
        icon: icons["rrtac_festivechaos"],
        maxRank: 1,
        reqPoints: 25,
        type: "renown tactic",
        description: talentText`You gain 2 stacks of Revelry each time you use an ability. Whenever you critically hit, you will lose all stacks of Revelry.\n\nRevelry: 1 adrenaline per second. Stacks 15 times and lasts 3 seconds.`,
      },
      "Fatass": {
        name: "Fatass",
        pos: "a8",
        icon: icons["rrtac_fatass"],
        maxRank: 1,
        reqPoints: 35,
        type: "renown tactic",
        description: talentText`Increases your mass by 1.`,
      },
      "Break Loose": {
        name: "Break Loose",
        pos: "a9",
        icon: icons["rrtac_breakloose"],
        maxRank: 1,
        reqPoints: 40,
        type: "renown tactic",
        description: talentText`Movement skills you use will now remove Ensnared and Rooted from you.`,
      },
      "Agility": {
        name: "Agility",
        pos: "b5",
        icon: icons["stat_agl"],
        maxRank: 1,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Agility by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Ballistic Skill": {
        name: "Ballistic Skill",
        pos: "b7",
        icon: icons["stat_bal"],
        maxRank: 3,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Ballistic Skill by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Initiative": {
        name: "Initiative",
        pos: "b3",
        icon: icons["stat_ini"],
        maxRank: 2,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Initiative by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Intelligence": {
        name: "Intelligence",
        pos: "e7",
        icon: icons["stat_int"],
        maxRank: 4,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Intelligence by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Strength": {
        name: "Strength",
        pos: "h7",
        icon: icons["stat_str"],
        maxRank: 3,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Strength by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Toughness": {
        name: "Toughness",
        pos: "e5",
        icon: icons["stat_tou"],
        maxRank: 3,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Toughness by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Willpower": {
        name: "Willpower",
        pos: "h3",
        icon: icons["stat_wil"],
        maxRank: 1,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Willpower by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Wounds": {
        name: "Wounds",
        pos: "h5",
        icon: icons["stat_wou"],
        maxRank: 3,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Wounds by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Weapon Skill": {
        name: "Weapon Skill",
        pos: "e3",
        icon: icons["stat_ws"],
        maxRank: 5,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Weapon Skill by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Fleet-Footed": {
        name: "Fleet-Footed",
        pos: "e4",
        icon: icons["tac_arche"],
        maxRank: 1,
        reqPoints: 15,
        type: "career tactic",
        description: talentText`Your base movement speed is increased by 15%. Your mass is decreased by 1.`,
      },
      "Morale 1 Spec": {
        name: "Morale 1s",
        pos: "b8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 1",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Morale 2 Spec": {
        name: "Morale 2s",
        pos: "h8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 2",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Morale 4 Spec": {
        name: "Morale 4s",
        pos: "i9",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 40,
        type: "morale 4",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 3600 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Emperor's Champion": {
        name: "Morale 3r",
        pos: "h6",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 25,
        type: "morale 3",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 1800 adrenaline",
        description: talentText`You gain a buff for 20 seconds. You gain 250 ballistic skill, 250 intelligence, and 250 strength.`,
      },
      "Exquisite Swordsmanship": {
        name: "Exquisitie Swordsmanship",
        pos: "h2",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 5,
        type: "morale 1",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`You gain a buff for 3 seconds that increases your dodge by 100.`,
      },
      "Multi-hit Combo": {
        name: "Multi-hit Combo",
        pos: "h4",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 15,
        type: "morale 2",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`Applies 5 combo points to up to 9 enemies within 30ft of you.`,
      },
      "Sever Nerve": {
        name: "Sever Nerve",
        pos: "i2",
        icon: icons["morale_severnerve"],
        maxRank: 1,
        reqPoints: 5,
        type: "morale 1",
        cost: "no cost",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`Deals 50% MAP + 400 physical damage to the target.`,
      },
      "Broad Swings": {
        name: "Broad Swings",
        pos: "i4",
        icon: icons["morale_broadswings"],
        maxRank: 1,
        reqPoints: 15,
        type: "morale 2",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`You gain a  buff for 10 seconds. Your single target abilities will target two additional enemies within 15ft in front of you.`,
      },
      "Force of Will": {
        name: "Force of Will",
        pos: "i6",
        icon: icons["morale_forceofwill"],
        maxRank: 1,
        reqPoints: 25,
        type: "morale 3",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 1800 adrenaline",
        description: talentText`All groupmates within 100ft gain a buff for 10 seconds. Action point costs are reduced by 75% for the duration.`,
      },
      "Frenzied Slaughter": {
        name: "Frenzied Slaughter",
        pos: "i8",
        icon: icons["morale_frenziedslaughter"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 4",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 3600 adrenaline",
        description: talentText`All groupmates within 100ft gain a buff for 15 seconds. +40% outgoing damage and 50% reduced cooldowns for the duration.`,
      },
      "Emperor's Ward": {
        name: "Emperor's Ward",
        pos: "a1",
        icon: icons["tac_race_empire"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you use a basic attack, you will gain a 400 absorb for 3 seconds.`,
      },
      "Sigmar's Favored": {
        name: "Sigmar's Favored",
        pos: "i1",
        icon: icons["tac_race_empire"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you receive a direct heal, you will also be healed for 120 + 8% wounds.`,
      },
      "Stalk": {
        name: "Stalk",
        pos: "b1",
        icon: icons["abi_stalk"],
        maxRank: 1,
        reqPoints: 0,
        type: "ability",
        cost: "no cost",
        range: "self",
        cast: "2.5s cast",
        cooldown: "45s cooldown",
        blue: "Castable while moving",
        description: talentText`(Movement Skill) You become invisible to enemies for up to 5 min. Your action points per second is set to -5, and you will come out of stalk if you reach 0 action points.`,
      },
      "Chaos Stalker": {
        name: "Chaos stalker",
        pos: "c1",
        icon: icons["abi_stalk"],
        maxRank: 2,
        reqPoints: 0,
        prereq: "Stalk",
        arrows: [{ dir: "right", from: "b1", to: "c1" }],
        type: "passive",
        description: talentText`While in Stalk, your movement speed is increased by ${[
          7,
          15,
        ]}%, and the AP cost of your Executes is reduced by ${[
          50,
          100,
        ]}%.\n\nCombo Points last 20s on the target and stack up to 5 times. Executes have their AP costs reduced by 20% per Combo Point on the target.`,
      },
      "Torment": {
        name: "Torment",
        pos: "d1",
        icon: icons["abi_torment"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "36 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Deals 22% MAP + 160 physical damage to target enemy. Generates 1 Combo Point on the target.`,
      },
      "Sanctioned Murder": {
        name: "Sanctioned Murder",
        pos: "e1",
        icon: icons["tac_class_1"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you critically hit an enemy, you will generate 1 Combo Point on them.`,
      },
      "Burn at the Stake": {
        name: "Burn at the Stake",
        pos: "f1",
        icon: icons["abi_burnstake"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "150 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Execute",
        description: talentText`Applies an affliction to target enemy for 10 seconds. Target takes 7% MAP + 70 physical damage every 2 seconds; the last tick will deal 30% MAP + 231 physical damage.`,
      },
      "Brutal Executions": {
        name: "Brutal Executions",
        pos: "f2",
        icon: icons["tac_spec_5"],
        maxRank: 1,
        reqPoints: 5,
        prereq: "Burn at the Stake",
        arrows: [{ dir: "down", from: "f1", to: "f2" }],
        type: "career tactic",
        description: talentText`Burn at the Stake and Rupture apply Off-balance to the target for 5 seconds.\n\nOff-balance suppresses the effects of Guard on the target.`,
      },
      "Strike": {
        name: "Strike",
        pos: "g1",
        icon: icons["abi_strike"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "no cost",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`(Basic Attack) Deals 120 physical damage to target enemy. Generates 22 action points.`,
      },
      "Improved Strike": {
        name: "Improved Strike",
        pos: "h1",
        icon: icons["abi_strike"],
        maxRank: 2,
        reqPoints: 0,
        prereq: "Strike",
        arrows: [{ dir: "right", from: "g1", to: "h1" }],
        type: "passive",
        description: talentText`Strike now deals additional damage equal to ${[
          4,
          8,
        ]}% MAP.`,
      },
      "Dual Strike": {
        name: "Dual Strike",
        pos: "g2",
        icon: icons["abi_dualstrike"],
        maxRank: 1,
        reqPoints: 5,
        prereq: "Strike",
        arrows: [{ dir: "down", from: "g1", to: "g2" }],
        type: "passive",
        description: talentText`Strike will now hit the target again for an additional 40% damage.`,
      },
      "Waylay": {
        name: "Waylay",
        pos: "c2",
        icon: icons["tac_class_2"],
        maxRank: 1,
        reqPoints: 5,
        type: "career tactic",
        description: talentText`Burn at the Stake, Rupture, Blessed Bullet, and Exit Wound have a 30% chance per Combo Point consumed to generate a Combo Point on the target.`,
      },
      "Vigilance": {
        name: "Vigilance",
        pos: "d2",
        icon: icons["abi_vigilance"],
        maxRank: 1,
        reqPoints: 5,
        type: "ability",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "60s cooldown",
        description: talentText`All damage you take is reduced by 50% but all damage you deal is reduced by 90% for 5 seconds.`,
      },
      "Improved Vigilance": {
        name: "Improved Vigilance",
        pos: "d4",
        icon: icons["abi_vigilance"],
        maxRank: 1,
        reqPoints: 15,
        prereq: "Vigilance",
        arrows: [{ dir: "down", from: "d2", to: "d4" }],
        type: "passive",
        description: talentText`Vigilance now only reduces your outgoing damage by 75%.`,
      },
      "Bind Weapon": {
        name: "Bind Weapon",
        pos: "e2",
        icon: icons["abi_bindweapon"],
        maxRank: 1,
        reqPoints: 5,
        type: "skill",
        cost: "30 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "10s cooldown",
        description: talentText`Applies Disarmed and Inevitable to target enemy.\n\nDisarmed prevents the use of skills for 5 seconds. Inevitable prevents the application of Disarmed and Silenced for 20 seconds.`,
      },
      "Razor Strike": {
        name: "Razor Strike",
        pos: "c4",
        icon: icons["abi_razorstrike"],
        maxRank: 1,
        reqPoints: 15,
        type: "skill",
        cost: "75 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Deals 10% MAP + 73 physical damage to up to 9 enemies within 30ft of you. You will generate a Combo Point on each target hit.`,
      },
      "Turn the Tables": {
        name: "Turn the Tables",
        pos: "f4",
        icon: icons["tac_spec_6"],
        maxRank: 1,
        reqPoints: 15,
        type: "career tactic",
        description: talentText`Whenever you dodge an enemy ability, you will generate 1 Combo Point on them and gain 8% wounds + 120 health.`,
      },
      "Fervor": {
        name: "Fervor",
        pos: "g4",
        icon: icons["abi_fervor"],
        maxRank: 1,
        reqPoints: 15,
        type: "skill",
        cost: "45 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Applies 1 stack of an affliction that lasts 9 seconds and generates 1 Combo Point on target enemy. Stacks up to 3 times, and deals 15% MAP + 84 physical damage every 3 seconds.`,
      },
      "Trial by Pain": {
        name: "Trial by Pain",
        pos: "c6",
        icon: icons["abi_trialbypain"],
        maxRank: 1,
        reqPoints: 25,
        type: "skill",
        cost: "150 action points",
        range: "7.5ft range",
        cast: "2.0s channel",
        cooldown: "no cooldown",
        blue: "Execute\nCastable while moving",
        description: talentText`Deals 6% MAP + 66 physical damage to target enemy and up to 8 other enemies within 30ft of them every second.`,
      },
      "Seal of Wilhelm": {
        name: "Seal of Wilhelm",
        pos: "d6",
        icon: icons["abi_wilhelmseal"],
        maxRank: 1,
        reqPoints: 25,
        type: "spell",
        cost: "30 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "30s cooldown",
        description: talentText`You gain a buff for the next 8 seconds. You gain 900 armor and 900 corporeal resistance but lose 900 elemental resistance and 900 spiritual resistance.`,
      },
      "Protection From Heresy": {
        name: "Protection From Heresy",
        pos: "d8",
        icon: icons["tac_spec_4"],
        maxRank: 1,
        reqPoints: 35,
        prereq: "Seal of Wilhelm",
        arrows: [{ dir: "down", from: "d6", to: "d8" }],
        type: "career tactic",
        description: talentText`Whenever you use a seal, you will gain a blessing for 10 seconds. You will absorb up to 30% RP + 600 damage.`,
      },
      "Seal of Magnus": {
        name: "Seal of Magnus",
        pos: "e6",
        icon: icons["abi_magnusseal"],
        maxRank: 1,
        reqPoints: 25,
        type: "spell",
        cost: "30 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "30s cooldown",
        description: talentText`You gain a buff for 8 seconds. Whenever an enemy hits you with a spell, they will be hit by the same spell.`,
      },
      "Necktie Party": {
        name: "Necktie Party",
        pos: "f6",
        icon: icons["tac_spec_8"],
        maxRank: 1,
        reqPoints: 25,
        type: "career tactic",
        description: talentText`Whenever you use an Execute, all groupmates within 100ft will gain 40 action points.`,
      },
      "Blood, Faith, and Fire": {
        name: "Blood, Faith, and Fire",
        pos: "g6",
        icon: icons["tac_spec_1"],
        maxRank: 1,
        reqPoints: 25,
        description: talentText`Rupture, Fervor, and Burn at the stake now apply cripples instead of afflictions and deal 10% more damage.`,
      },
      "Unfair Advantage": {
        name: "Unfair Advantage",
        pos: "c8",
        icon: icons["tac_spec_2"],
        maxRank: 1,
        reqPoints: 35,
        type: "career tactic",
        description: talentText`Whenever you are flanking a target, you will deal additional damage equal to 50% of your accuracy.`,
      },
      "Smash": {
        name: "Smash",
        pos: "e8",
        icon: icons["abi_smash"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "60 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "20s cooldown",
        description: talentText`Applies Knocked-down and Unstoppable to target enemy.\n\nKnocked-down prevents all control of your character for 3 seconds. Unstoppable prevents application of Knocked-down and Staggered for 30 seconds.`,
      },
      "Righteous Steel": {
        name: "Righteous Steel",
        pos: "f8",
        icon: icons["tac_spec_7"],
        maxRank: 1,
        reqPoints: 35,
        prereq: "Smash",
        arrows: [{ dir: "right", from: "e8", to: "f8" }],
        type: "career tactic",
        description: talentText`Bind Weapon, Silence Heretic, and Smash will now also deal 22% MAP + 160 physical damage to the target and generate 1 Combo Point on them.`,
      },
      "Rupture": {
        name: "Rupture",
        pos: "g8",
        icon: icons["abi_rupture"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "150 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Execute",
        description: talentText`Apply an affliction to target enemy for 15 seconds. Deals 20% MAP + 175 physical damage every 3 seconds.`,
      },
      "Skewer": {
        name: "Skewer",
        pos: "d9",
        icon: icons["abi_skewer"],
        maxRank: 1,
        reqPoints: 40,
        type: "skill",
        cost: "39 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Off the GCD",
        description: talentText`Deals 10% MAP + 112 physical damage to target enemy. Generates 1 Combo Point.`,
      },
      "Zerg Buster": {
        name: "Zerg Buster",
        pos: "f9",
        icon: icons["abi_zergbuster"],
        maxRank: 1,
        reqPoints: 40,
        type: "skill",
        cost: "150 action points",
        range: "self",
        cast: "5.0s channel",
        cooldown: "30s cooldown",
        description: talentText`Deal 10% MAP + 50 physical damage to up to 48 enemies within 30ft of you every second. Each enemy hit will increase your outgoing damage by 10% up to a maximum of 100 stacks. Effect is dispelled when channel ends.`,
      },
    },
  },
  "Witch Hunter": {
    name: "Witch Hunter",
    background: backgrounds["witchhunter"],
    icon: icons["archetype_dps"],
    talents: {
      "Motivated": {
        name: "Motivated",
        pos: "a2",
        icon: icons["rrtac_motivated"],
        maxRank: 1,
        reqPoints: 5,
        type: "renown tactic",
        description: talentText`You gain 5 adrenaline every second while in combat.`,
      },
      "Strength in Numbers": {
        name: "Strength in Numbers",
        pos: "b2",
        icon: icons["rrtac_strengthinnumbers"],
        maxRank: 1,
        reqPoints: 5,
        type: "renown tactic",
        description: talentText`Whenever you use a basic attack, all groupmates within 100 feet that also have this tactic will gain a stack. Max 5 stacks, each stack grants 2 adrenaline per second for a duration of 5 seconds.`,
      },
      "Uncontrolled": {
        name: "Uncontrolled",
        pos: "a4",
        icon: icons["rrtac_uncontrolled"],
        maxRank: 1,
        reqPoints: 15,
        type: "renown tactic",
        description: talentText`Whenever Unstoppable, Inevitable, or Immovable is applied to you, you will gain 20 adrenaline per second over 5 seconds.`,
      },
      "Stay in Formation": {
        name: "Stay in Formation",
        pos: "b4",
        icon: icons["rrtac_stayinformation"],
        maxRank: 1,
        reqPoints: 15,
        type: "renown tactic",
        description: talentText`Whenever you are attacked by an opponent you have Cover from, you will gain 10 adrenaline per second for 3 seconds.`,
      },
      "Gotta Go Fast": {
        name: "Gotta Go Fast",
        pos: "b6",
        icon: icons["rrtac_gottagofast"],
        maxRank: 1,
        reqPoints: 25,
        type: "renown tactic",
        description: talentText`You gain 1 stack of Revelry every second while moving.\n\nRevelry: 1 adrenaline per second. Stacks 15 times and lasts 3 seconds.`,
      },
      "Festive Chaos": {
        name: "Festivity",
        pos: "a6",
        icon: icons["rrtac_festivechaos"],
        maxRank: 1,
        reqPoints: 25,
        type: "renown tactic",
        description: talentText`You gain 2 stacks of Revelry each time you use an ability. Whenever you critically hit, you will lose all stacks of Revelry.\n\nRevelry: 1 adrenaline per second. Stacks 15 times and lasts 3 seconds.`,
      },
      "Fatass": {
        name: "Fatass",
        pos: "a8",
        icon: icons["rrtac_fatass"],
        maxRank: 1,
        reqPoints: 35,
        type: "renown tactic",
        description: talentText`Increases your mass by 1.`,
      },
      "Break Loose": {
        name: "Break Loose",
        pos: "a9",
        icon: icons["rrtac_breakloose"],
        maxRank: 1,
        reqPoints: 40,
        type: "renown tactic",
        description: talentText`Movement skills you use will now remove Ensnared and Rooted from you.`,
      },
      "Agility": {
        name: "Agility",
        pos: "b5",
        icon: icons["stat_agl"],
        maxRank: 1,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Agility by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Ballistic Skill": {
        name: "Ballistic Skill",
        pos: "b7",
        icon: icons["stat_bal"],
        maxRank: 5,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Ballistic Skill by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Initiative": {
        name: "Initiative",
        pos: "b3",
        icon: icons["stat_ini"],
        maxRank: 2,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Initiative by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Intelligence": {
        name: "Intelligence",
        pos: "e7",
        icon: icons["stat_int"],
        maxRank: 3,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Intelligence by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Strength": {
        name: "Strength",
        pos: "h7",
        icon: icons["stat_str"],
        maxRank: 3,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Strength by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Toughness": {
        name: "Toughness",
        pos: "e5",
        icon: icons["stat_tou"],
        maxRank: 3,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Toughness by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Willpower": {
        name: "Willpower",
        pos: "h3",
        icon: icons["stat_wil"],
        maxRank: 1,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Willpower by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Wounds": {
        name: "Wounds",
        pos: "h5",
        icon: icons["stat_wou"],
        maxRank: 3,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Wounds by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Weapon Skill": {
        name: "Weapon Skill",
        pos: "e3",
        icon: icons["stat_ws"],
        maxRank: 4,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Weapon Skill by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Fleet-Footed": {
        name: "Fleet-Footed",
        pos: "e4",
        icon: icons["tac_arche"],
        maxRank: 1,
        reqPoints: 15,
        type: "career tactic",
        description: talentText`Your base movement speed is increased by 15%. Your mass is decreased by 1.`,
      },
      "Morale 1 Spec": {
        name: "Morale 1s",
        pos: "b8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 1",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Morale 2 Spec": {
        name: "Morale 2s",
        pos: "h8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 2",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Morale 4 Spec": {
        name: "Morale 4s",
        pos: "i9",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 40,
        type: "morale 4",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 3600 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Emperor's Champion": {
        name: "Morale 3r",
        pos: "h6",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 25,
        type: "morale 3",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 1800 adrenaline",
        description: talentText`You gain a buff for 20 seconds. You gain 250 ballistic skill, 250 intelligence, and 250 strength.`,
      },
      "Exquisite Swordsmanship": {
        name: "Exquisitie Swordsmanship",
        pos: "h2",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 5,
        type: "morale 1",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`You gain a buff for 3 seconds that increases your dodge by 100.`,
      },
      "Multi-hit Combo": {
        name: "Multi-hit Combo",
        pos: "h4",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 15,
        type: "morale 2",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`Applies 5 combo points to up to 9 enemies within 30ft of you.`,
      },
      "Sever Nerve": {
        name: "Sever Nerve",
        pos: "i2",
        icon: icons["morale_severnerve"],
        maxRank: 1,
        reqPoints: 5,
        type: "morale 1",
        cost: "no cost",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`Deals 50% MAP + 400 physical damage to the target.`,
      },
      "Broad Swings": {
        name: "Broad Swings",
        pos: "i4",
        icon: icons["morale_broadswings"],
        maxRank: 1,
        reqPoints: 15,
        type: "morale 2",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`You gain a  buff for 10 seconds. Your single target abilities will target two additional enemies within 15ft in front of you.`,
      },
      "Force of Will": {
        name: "Force of Will",
        pos: "i6",
        icon: icons["morale_forceofwill"],
        maxRank: 1,
        reqPoints: 25,
        type: "morale 3",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 1800 adrenaline",
        description: talentText`All groupmates within 100ft gain a buff for 10 seconds. Action point costs are reduced by 75% for the duration.`,
      },
      "Frenzied Slaughter": {
        name: "Frenzied Slaughter",
        pos: "i8",
        icon: icons["morale_frenziedslaughter"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 4",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 3600 adrenaline",
        description: talentText`All groupmates within 100ft gain a buff for 15 seconds. +40% outgoing damage and 50% reduced cooldowns for the duration.`,
      },
      "Emperor's Ward": {
        name: "Emperor's Ward",
        pos: "a1",
        icon: icons["tac_race_empire"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you use a basic attack, you will gain a 400 absorb for 3 seconds.`,
      },
      "Sigmar's Favored": {
        name: "Sigmar's Favored",
        pos: "i1",
        icon: icons["tac_race_empire"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you receive a direct heal, you will also be healed for 120 + 8% wounds.`,
      },
      "Stalk": {
        name: "Stalk",
        pos: "b1",
        icon: icons["abi_stalk"],
        maxRank: 1,
        reqPoints: 0,
        type: "ability",
        cost: "no cost",
        range: "self",
        cast: "2.5s cast",
        cooldown: "45s cooldown",
        blue: "Castable while moving",
        description: talentText`(Movement Skill) You become invisible to enemies for up to 5 min. Your action points per second is set to -5, and you will come out of stalk if you reach 0 action points.`,
      },
      "Chaos Stalker": {
        name: "Chaos stalker",
        pos: "c1",
        icon: icons["abi_stalk"],
        maxRank: 2,
        reqPoints: 0,
        prereq: "Stalk",
        arrows: [{ dir: "right", from: "b1", to: "c1" }],
        type: "passive",
        description: talentText`While in Stalk, your movement speed is increased by ${[
          7,
          15,
        ]}%, and the AP cost of your Executes is reduced by ${[
          50,
          100,
        ]}%.\n\nCombo Points last 20s on the target and stack up to 5 times. Executes have their AP costs reduced by 20% per Combo Point on the target.`,
      },
      "Torment": {
        name: "Torment",
        pos: "d1",
        icon: icons["abi_torment"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "36 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Deals 22% MAP + 160 physical damage to target enemy. Generates 1 Combo Point on the target.`,
      },
      "Sanctioned Murder": {
        name: "Sanctioned Murder",
        pos: "e1",
        icon: icons["tac_class_1"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you critically hit an enemy, you will generate 1 Combo Point on them.`,
      },
      "Burn at the Stake": {
        name: "Burn at the Stake",
        pos: "f1",
        icon: icons["abi_burnstake"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "150 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Execute",
        description: talentText`Applies an affliction to target enemy for 10 seconds. Target takes 7% MAP + 70 physical damage every 2 seconds; the last tick will deal 30% MAP + 231 physical damage.`,
      },
      "Strike": {
        name: "Strike",
        pos: "g1",
        icon: icons["abi_strike"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "no cost",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`(Basic Attack) Deals 120 physical damage to target enemy. Generates 22 action points.`,
      },
      "Dual Strike": {
        name: "Dual Strike",
        pos: "g2",
        icon: icons["abi_dualstrike"],
        maxRank: 1,
        reqPoints: 5,
        prereq: "Strike",
        arrows: [{ dir: "down", from: "g1", to: "g2" }],
        type: "passive",
        description: talentText`Strike will now hit the target again for an additional 40% damage.`,
      },
      "Men of Action": {
        name: "Men of Action",
        pos: "h1",
        icon: icons["abi_menofaction"],
        maxRank: 2,
        reqPoints: 0,
        type: "passive",
        description: talentText`Reduces the cast times of Blessed Bullet, Exit Wound, and Silence Heretic by ${[
          0.75,
          1.5,
        ]} seconds.`,
      },
      "Waylay": {
        name: "Waylay",
        pos: "c2",
        icon: icons["tac_class_2"],
        maxRank: 1,
        reqPoints: 5,
        type: "career tactic",
        description: talentText`Burn at the Stake, Rupture, Blessed Bullet, and Exit Wound have a 30% chance per Combo Point consumed to generate a Combo Point on the target.`,
      },
      "Vigilance": {
        name: "Vigilance",
        pos: "d2",
        icon: icons["abi_vigilance"],
        maxRank: 1,
        reqPoints: 5,
        type: "ability",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "60s cooldown",
        description: talentText`All damage you take is reduced by 50% but all damage you deal is reduced by 90% for 5 seconds.`,
      },
      "Improved Vigilance": {
        name: "Improved Vigilance",
        pos: "d4",
        icon: icons["abi_vigilance"],
        maxRank: 1,
        reqPoints: 15,
        prereq: "Vigilance",
        arrows: [{ dir: "down", from: "d2", to: "d4" }],
        type: "passive",
        description: talentText`Vigilance now only reduces your outgoing damage by 75%.`,
      },
      "Blessed Bullet": {
        name: "Blessed Bullet",
        pos: "e2",
        icon: icons["abi_blessedbullet"],
        maxRank: 1,
        reqPoints: 5,
        type: "skill",
        cost: "150 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "no cooldown",
        blue: "Execute",
        description: talentText`Deals 50% RAP + 472 physical damage to target enemy.`,
      },
      "Vitriolic Judgement": {
        name: "Vitriolic Judgement",
        pos: "f2",
        icon: icons["tac_spec_3"],
        maxRank: 1,
        reqPoints: 5,
        type: "career tactic",
        description: talentText`You have a 100% chance whenever you get a killing blow and a 33% chance whenever you use an execute to reset the cooldown on Stalk.`,
      },
      "Razor Strike": {
        name: "Razor Strike",
        pos: "c4",
        icon: icons["abi_razorstrike"],
        maxRank: 1,
        reqPoints: 15,
        type: "skill",
        cost: "75 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Deals 10% MAP + 73 physical damage to up to 9 enemies within 30ft of you. You will generate a Combo Point on each target hit.`,
      },
      "Encourage Confession": {
        name: "Encourage Confession",
        pos: "f4",
        icon: icons["tac_spec_2"],
        maxRank: 1,
        reqPoints: 15,
        type: "career tactic",
        description: talentText`Whenever you hit an enemy with an Execute, they will be crippled for 6 seconds reducing their block rate by 30%`,
      },
      "Silence Heretic": {
        name: "Silence Heretic",
        pos: "g4",
        icon: icons["abi_silenceheretic"],
        maxRank: 1,
        reqPoints: 15,
        type: "skill",
        cost: "40 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "10s cooldown",
        description: talentText`Applies Silenced and Inevitable to target enemy.\n\nSilenced prevents the use of spells for 5 seconds. Inevitable prevents the application of Disarmed and Silenced.`,
      },
      "Trial by Pain": {
        name: "Trial by Pain",
        pos: "c6",
        icon: icons["abi_trialbypain"],
        maxRank: 1,
        reqPoints: 25,
        type: "skill",
        cost: "150 action points",
        range: "7.5ft range",
        cast: "2.0s channel",
        cooldown: "no cooldown",
        blue: "Execute\nCastable while moving",
        description: talentText`Deals 6% MAP + 66 physical damage to target enemy and up to 8 other enemies within 30ft of them every second.`,
      },
      "Seal of Wilhelm": {
        name: "Seal of Wilhelm",
        pos: "d6",
        icon: icons["abi_wilhelmseal"],
        maxRank: 1,
        reqPoints: 25,
        type: "spell",
        cost: "30 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "30s cooldown",
        description: talentText`You gain a buff for the next 8 seconds. You gain 900 armor and 900 corporeal resistance but lose 900 elemental resistance and 900 spiritual resistance.`,
      },
      "Sanitation Project": {
        name: "Sanitation Project",
        pos: "d8",
        icon: icons["tac_spec_6"],
        maxRank: 1,
        reqPoints: 35,
        prereq: "Seal of Wilhelm",
        arrows: [{ dir: "down", from: "d6", to: "d8" }],
        type: "career tactic",
        description: talentText`Seal of Wilhelm will no longer increase your armor and corporeal resistance. Instead it will apply a 8 second duration ailment to up to 48 enemies within 30ft of you that reduces armor and corporeal resistance by 600.`,
      },
      "Seal of Van Horstmann": {
        name: "Seal of Van Horstmann",
        pos: "e6",
        icon: icons["abi_horstmannseal"],
        maxRank: 1,
        reqPoints: 25,
        type: "spell",
        cost: "30 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "30s cooldown",
        description: talentText`You gain a buff for the next 8 seconds. You gain 900 elemental resistance and 900 spiritual resistance but lose 900 armor and 900 corporeal resistance.`,
      },
      "Find Weakness": {
        name: "Find Weakness",
        pos: "f6",
        icon: icons["tac_spec_5"],
        maxRank: 1,
        reqPoints: 25,
        prereq: "Stalk",
        type: "career tactic",
        description: talentText`While you are in Stalk, you gain a blessing which lasts for 10 seconds after you leave Stalk. For the duration, you gain 15% critical chance.`,
      },
      "No One Expects the Inquisition": {
        name: "No One Expects the Inquisition",
        pos: "g6",
        icon: icons["tac_spec_8"],
        maxRank: 1,
        reqPoints: 25,
        type: "career tactic",
        description: talentText`The first time you hit a target, you will generate 5 COmbo Points on them. This effect can only happen once every 20 seconds.`,
      },
      "Unfair Advantage": {
        name: "Unfair Advantage",
        pos: "c8",
        icon: icons["tac_spec_2"],
        maxRank: 1,
        reqPoints: 35,
        type: "career tactic",
        description: talentText`Whenever you are flanking a target, you will deal additional damage equal to 50% of your accuracy.`,
      },
      "Smash": {
        name: "Smash",
        pos: "e8",
        icon: icons["abi_smash"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "60 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "20s cooldown",
        description: talentText`Applies Knocked-down and Unstoppable to target enemy.\n\nKnocked-down prevents all control of your character for 3 seconds. Unstoppable prevents application of Knocked-down and Staggered for 30 seconds.`,
      },
      "Righteous Steel": {
        name: "Righteous Steel",
        pos: "f8",
        icon: icons["tac_spec_7"],
        maxRank: 1,
        reqPoints: 35,
        prereq: "Smash",
        arrows: [{ dir: "right", from: "e8", to: "f8" }],
        type: "career tactic",
        description: talentText`Bind Weapon, Silence Heretic, and Smash will now also deal 22% MAP + 160 physical damage to the target and generate 1 Combo Point on them.`,
      },
      "Exit Wound": {
        name: "Exit Wound",
        pos: "g8",
        icon: icons["abi_exitwound"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "50 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "no cooldown",
        description: talentText`Deals 18% RAP + 138 physical damage to the target and gives them 1 stack of Reduced Healing Received. Generates 1 Combo Point.`,
      },
      "Feint and Reposition": {
        name: "Feint and Reposition",
        pos: "d9",
        icon: icons["abi_feint"],
        maxRank: 1,
        reqPoints: 40,
        type: "ability",
        cost: "no cost",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "60s cooldown",
        description: talentText`Knock yourself back away from target enemy. Remove all Combo Points from each enemy within 50ft.`,
      },
      "Dragon Gun": {
        name: "Dragon Gun",
        pos: "f9",
        icon: icons["abi_dragongun"],
        maxRank: 1,
        reqPoints: 40,
        type: "skill",
        cost: "150 action points",
        range: "self",
        cast: "2.0s cast",
        cooldown: "30s cooldown",
        blue: "Castable while moving",
        description: talentText`Applies an 8 second duration cripple and 5 Combo Points to up to 9 enemies 30ft in front of you. Each second the target and up to 9 enemies within 20ft of them will take 10% RAP + 100 elemental damage.`,
      },
    },
  },
  "Outrider": {
    name: "Outrider",
    background: backgrounds["outrider"],
    icon: icons["archetype_offsupport"],
    talents: {
      "Motivated": {
        name: "Motivated",
        pos: "a2",
        icon: icons["rrtac_motivated"],
        maxRank: 1,
        reqPoints: 5,
        type: "renown tactic",
        description: talentText`You gain 5 adrenaline every second while in combat.`,
      },
      "Strength in Numbers": {
        name: "Strength in Numbers",
        pos: "b2",
        icon: icons["rrtac_strengthinnumbers"],
        maxRank: 1,
        reqPoints: 5,
        type: "renown tactic",
        description: talentText`Whenever you use a basic attack, all groupmates within 100 feet that also have this tactic will gain a stack. Max 5 stacks, each stack grants 2 adrenaline per second for a duration of 5 seconds.`,
      },
      "Uncontrolled": {
        name: "Uncontrolled",
        pos: "a4",
        icon: icons["rrtac_uncontrolled"],
        maxRank: 1,
        reqPoints: 15,
        type: "renown tactic",
        description: talentText`Whenever Unstoppable, Inevitable, or Immovable is applied to you, you will gain 20 adrenaline per second over 5 seconds.`,
      },
      "Stay in Formation": {
        name: "Stay in Formation",
        pos: "b4",
        icon: icons["rrtac_stayinformation"],
        maxRank: 1,
        reqPoints: 15,
        type: "renown tactic",
        description: talentText`Whenever you are attacked by an opponent you have Cover from, you will gain 10 adrenaline per second for 3 seconds.`,
      },
      "Gotta Go Fast": {
        name: "Gotta Go Fast",
        pos: "b6",
        icon: icons["rrtac_gottagofast"],
        maxRank: 1,
        reqPoints: 25,
        type: "renown tactic",
        description: talentText`You gain 1 stack of Revelry every second while moving.\n\nRevelry: 1 adrenaline per second. Stacks 15 times and lasts 3 seconds.`,
      },
      "Festive Chaos": {
        name: "Festivity",
        pos: "a6",
        icon: icons["rrtac_festivechaos"],
        maxRank: 1,
        reqPoints: 25,
        type: "renown tactic",
        description: talentText`You gain 2 stacks of Revelry each time you use an ability. Whenever you critically hit, you will lose all stacks of Revelry.\n\nRevelry: 1 adrenaline per second. Stacks 15 times and lasts 3 seconds.`,
      },
      "Fatass": {
        name: "Fatass",
        pos: "a8",
        icon: icons["rrtac_fatass"],
        maxRank: 1,
        reqPoints: 35,
        type: "renown tactic",
        description: talentText`Increases your mass by 1.`,
      },
      "Verge of Victory": {
        name: "Verge of Victory",
        pos: "a9",
        icon: icons["rrtac_vergeofvictory"],
        maxRank: 1,
        reqPoints: 40,
        type: "renown tactic",
        description: talentText`Whenever you land a killing blow, you will gain 20 adrenaline per second for 8 seconds. Whenever a groupmate within 100ft lands a killing blow, you will gain 5 adrenaline per second for 8 seconds.`,
      },
      "Agility": {
        name: "Agility",
        pos: "b5",
        icon: icons["stat_agl"],
        maxRank: 2,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Agility by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Ballistic Skill": {
        name: "Ballistic Skill",
        pos: "b7",
        icon: icons["stat_bal"],
        maxRank: 5,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Ballistic Skill by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Initiative": {
        name: "Initiative",
        pos: "b3",
        icon: icons["stat_ini"],
        maxRank: 2,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Initiative by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Intelligence": {
        name: "Intelligence",
        pos: "e7",
        icon: icons["stat_int"],
        maxRank: 3,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Intelligence by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Strength": {
        name: "Strength",
        pos: "h7",
        icon: icons["stat_str"],
        maxRank: 3,
        reqPoints: 30,
        type: "passive",
        description: talentText`Increases your Strength by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Toughness": {
        name: "Toughness",
        pos: "e5",
        icon: icons["stat_tou"],
        maxRank: 3,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Toughness by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Willpower": {
        name: "Willpower",
        pos: "h3",
        icon: icons["stat_wil"],
        maxRank: 1,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Willpower by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Wounds": {
        name: "Wounds",
        pos: "h5",
        icon: icons["stat_wou"],
        maxRank: 2,
        reqPoints: 20,
        type: "passive",
        description: talentText`Increases your Wounds by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Weapon Skill": {
        name: "Weapon Skill",
        pos: "e3",
        icon: icons["stat_ws"],
        maxRank: 4,
        reqPoints: 10,
        type: "passive",
        description: talentText`Increases your Weapon Skill by ${[
          40,
          95,
          165,
          250,
          350,
        ]}.`,
      },
      "Fleet-Footed": {
        name: "Fleet-Footed",
        pos: "e4",
        icon: icons["tac_arche"],
        maxRank: 1,
        reqPoints: 15,
        type: "career tactic",
        description: talentText`Your base movement speed is increased by 15%. Your mass is decreased by 1.`,
      },
      "Morale 1 Spec": {
        name: "Morale 1s",
        pos: "b8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 1",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Morale 2 Spec": {
        name: "Morale 2s",
        pos: "h8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 2",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Morale 4 Spec": {
        name: "Morale 4s",
        pos: "i9",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 40,
        type: "morale 4",
        cost: "no cost",
        range: "7.5ft range self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 3600 adrenaline",
        description: talentText`Insert tooltip.`,
      },
      "Emperor's Champion": {
        name: "Morale 3r",
        pos: "h6",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 25,
        type: "morale 3",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 1800 adrenaline",
        description: talentText`You gain a buff for 20 seconds. You gain 250 ballistic skill, 250 intelligence, and 250 strength.`,
      },
      "Exquisite Swordsmanship": {
        name: "Exquisitie Swordsmanship",
        pos: "h2",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 5,
        type: "morale 1",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`You gain a buff for 3 seconds that increases your dodge by 100.`,
      },
      "Multi-hit Combo": {
        name: "Multi-hit Combo",
        pos: "h4",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 15,
        type: "morale 2",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`Applies 5 combo points to up to 9 enemies within 30ft of you.`,
      },
      "Sever Nerve": {
        name: "Sever Nerve",
        pos: "i2",
        icon: icons["morale_severnerve"],
        maxRank: 1,
        reqPoints: 5,
        type: "morale 1",
        cost: "no cost",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 360 adrenaline",
        description: talentText`Deals 50% MAP + 400 physical damage to the target.`,
      },
      "Focused Mind": {
        name: "Focused Mind",
        pos: "i4",
        icon: icons["morale_focusedmind"],
        maxRank: 1,
        reqPoints: 15,
        type: "morale 2",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 720 adrenaline",
        description: talentText`You gain a buff for 15 seconds. Abilities will have 50% faster cast time and you will have 100% pushback immunity.`,
      },
      "Explosive Impact": {
        name: "Explosive Impact",
        pos: "i6",
        icon: icons["morale_explosiveimpact"],
        maxRank: 1,
        reqPoints: 25,
        type: "morale 3",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 1800 adrenaline",
        description: talentText`You gain a buff for 10 seconds. Whenever you hit a target with a single-target ability, you will deal 10% RAP + 150 corporeal damage and 10% SAP + 150 elemental damage to up to 24 enemies within 20ft of the target.`,
      },
      "Frenzied Slaughter": {
        name: "Frenzied Slaughter",
        pos: "i8",
        icon: icons["morale_frenziedslaughter"],
        maxRank: 1,
        reqPoints: 35,
        type: "morale 4",
        cost: "no cost",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        blue: "Costs 3600 adrenaline",
        description: talentText`All groupmates within 100ft gain a buff for 15 seconds. +40% outgoing damage and 50% reduced cooldowns for the duration.`,
      },
      "Emperor's Ward": {
        name: "Emperor's Ward",
        pos: "a1",
        icon: icons["tac_race_empire"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you use a basic attack, you will gain a 400 absorb for 3 seconds.`,
      },
      "Sigmar's Favored": {
        name: "Sigmar's Favored",
        pos: "i1",
        icon: icons["tac_race_empire"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you receive a direct heal, you will also be healed for 120 + 8% wounds.`,
      },
      "Mount Warhorse": {
        name: "Mount Warhorse",
        pos: "b1",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 0,
        type: "ability",
        cost: "no cost",
        range: "self",
        cast: "3.0s cast",
        cooldown: "15s cooldown",
        blue: "Castable while moving",
        description: talentText`Toggle: You gain 25% movement speed, -30% accuracy, and 1.0s cast time reduction.`,
      },
      "Expedited Mounting": {
        name: "Expedited Mounting",
        pos: "b2",
        icon: icons["tac_spec_3"],
        maxRank: 1,
        reqPoints: 0,
        prereq: "Mount Warhorse",
        arrows: [{ dir: "right", from: "b1", to: "b2" }],
        type: "career tactic",
        description: talentText`Mount Warhorse now has a 1.5 second cast time, but will go on a 30 second cooldown when removed.`,
      },
      "Suppressive Fire": {
        name: "Suppressive Fire",
        pos: "d1",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "50 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "no cooldown",
        description: talentText`Deals 25% RAP + 234 physical damage to target enemy and applies a 1 second debuff. Adrenaline per second is reduced by 10. Generates 1 Combo Point on the target.`,
      },
      "Take Aim": {
        name: "Take Aim",
        pos: "d4",
        icon: icons["tac_spec_1"],
        maxRank: 1,
        reqPoints: 15,
        prereq: "Suppressive Fire",
        arrows: [{ dir: "down", from: "d1", to: "d4" }],
        description: talentText`All of your non-instant cast abilities have their cast times increased by 1.0 seconds. Your accuracy is increased by 15% and your outgoing damage is increased by 30%.`,
      },
      "Sanctioned Murder": {
        name: "Sanctioned Murder",
        pos: "e1",
        icon: icons["tac_class_1"],
        maxRank: 1,
        reqPoints: 0,
        type: "career tactic",
        description: talentText`Whenever you critically hit an enemy, you will generate 1 Combo Point on them.`,
      },
      "Blessed Bullet": {
        name: "Blessed Bullet",
        pos: "f1",
        icon: icons["abi_blessedbullet"],
        maxRank: 1,
        reqPoints: 5,
        type: "skill",
        cost: "150 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "no cooldown",
        blue: "Execute",
        description: talentText`Deals 50% RAP + 472 physical damage to target enemy.`,
      },
      "Concentrated Fire": {
        name: "Concentrated Fire",
        pos: "f2",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 5,
        prereq: "Blessed Bullet",
        arrows: [{ dir: "down", from: "f1", to: "f2" }],
        type: "passive",
        description: talentText`Blessed Bullet applies an ailment to the target with a 10s duration that increases the damage of Blessed Bullet on the target by 25%.`,
      },
      "Steel and Gunpowder": {
        name: "Steel and Gunpowder",
        pos: "g1",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 0,
        type: "skill",
        cost: "no cost",
        range: "50ft range",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Deals 72 physical damage to target enemy. Deals an additional 48 physical damage as a second hit if you are within 5ft of them. Generates 22 action points.`,
      },
      "Superior Powder": {
        name: "Superior Powder",
        pos: "h1",
        icon: icons["noicon"],
        maxRank: 2,
        reqPoints: 0,
        prereq: "Steel and Gunpowder",
        arrows: [{ dir: "right", from: "g1", to: "h1" }],
        type: "passive",
        description: talentText`The initial hit of Steel and Gunpowder deals additional damage equal to ${[
          4,
          8,
        ]}% RAP.`,
      },
      "Superior Steel": {
        name: "Superior Steel",
        pos: "g2",
        icon: icons["noicon"],
        maxRank: 2,
        reqPoints: 5,
        prereq: "Steel and Gunpowder",
        arrows: [{ dir: "down", from: "g1", to: "g2" }],
        type: "passive",
        description: talentText`The second hit of Steel and Gunpowder deals additional damage equal to ${[
          4,
          8,
        ]}% MAP.`,
      },
      "Waylay": {
        name: "Waylay",
        pos: "c2",
        icon: icons["tac_class_2"],
        maxRank: 1,
        reqPoints: 5,
        type: "career tactic",
        description: talentText`Burn at the Stake, Rupture, Blessed Bullet, and Exit Wound have a 30% chance per Combo Point consumed to generate a Combo Point on the target.`,
      },
      "Smokescreen": {
        name: "Smokescreen",
        pos: "d2",
        icon: icons["abi_smokescreen"],
        maxRank: 1,
        reqPoints: 5,
        type: "ability",
        cost: "no cost",
        range: "150ft range",
        cast: "instant cast",
        cooldown: "15s cooldown",
        description: talentText`Target enemy will deal 50% less damage to you for 15 seconds. This effect is removed if you hit the target with an ability.`,
      },
      "Razor Strike": {
        name: "Razor Strike",
        pos: "c4",
        icon: icons["abi_razorstrike"],
        maxRank: 1,
        reqPoints: 15,
        type: "skill",
        cost: "75 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "no cooldown",
        description: talentText`Deals 10% MAP + 73 physical damage to up to 9 enemies within 30ft of you. You will generate a Combo Point on each target hit.`,
      },
      "Encourage Confession": {
        name: "Encourage Confession",
        pos: "f4",
        icon: icons["tac_spec_2"],
        maxRank: 1,
        reqPoints: 15,
        type: "career tactic",
        description: talentText`Whenever you hit an enemy with an Execute, they will be crippled for 6 seconds reducing their block rate by 30%.`,
      },
      "Silence Heretic": {
        name: "Silence Heretic",
        pos: "g4",
        icon: icons["abi_silenceheretic"],
        maxRank: 1,
        reqPoints: 15,
        type: "skill",
        cost: "40 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "10s cooldown",
        description: talentText`Applies Silenced and Inevitable to target enemy.\n\nSilenced prevents the use of spells for 5 seconds. Inevitable prevents the application of Disarmed and Silenced.`,
      },
      "Trial by Pain": {
        name: "Trial by Pain",
        pos: "c6",
        icon: icons["abi_trialbypain"],
        maxRank: 1,
        reqPoints: 25,
        type: "skill",
        cost: "150 action points",
        range: "7.5ft range",
        cast: "2.0s channel",
        cooldown: "no cooldown",
        blue: "Execute\nCastable while moving",
        description: talentText`Deals 6% MAP + 66 physical damage to target enemy and up to 8 other enemies within 30ft of them every second.`,
      },
      "Seal of Magnus": {
        name: "Seal of Magnus",
        pos: "d6",
        icon: icons["abi_magnusseal"],
        maxRank: 1,
        reqPoints: 25,
        type: "spell",
        cost: "30 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "30s cooldown",
        description: talentText`You gain a buff for 8 seconds. Whenever an enemy hits you with a spell, they will be hit by the same spell.`,
      },
      "Seal of Van Horstmann": {
        name: "Seal of Horstmann",
        pos: "e6",
        icon: icons["abi_horstmannseal"],
        maxRank: 1,
        reqPoints: 25,
        type: "spell",
        cost: "30 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "30s cooldown",
        description: talentText`You gain a buff for the next 8 seconds. You gain 900 elemental resistance and 900 spiritual resistance but lose 900 armor and 900 corporeal resistance.`,
      },
      "Protection From Heresy": {
        name: "Protection From Heresy",
        pos: "c8",
        icon: icons["tac_spec_4"],
        maxRank: 1,
        reqPoints: 35,
        type: "career tactic",
        description: talentText`Whenever you use a seal, you will gain a blessing for 10 seconds. You will absorb up to 30% RP + 600 damage.`,
      },
      "Exit Wound": {
        name: "Exit Wound",
        pos: "f6",
        icon: icons["abi_exitwound"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "50 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "no cooldown",
        description: talentText`Deals 18% RAP + 138 physical damage to the target and gives them 1 stack of Reduced Healing Received. Generates 1 Combo Point.`,
      },
      "Fragmented Exit": {
        name: "Fragmented Exit",
        pos: "g6",
        icon: icons["tac_spec_6"],
        maxRank: 1,
        reqPoints: 25,
        prereq: "Exit Wound",
        arrows: [{ dir: "right", from: "f6", to: "f7" }],
        type: "career tactic",
        description: talentText`Exit Wound will now hit up to 9 other enemies within 20ft of the target for 33% damage as well as applying stacks.`,
      },
      "Hollowed Bullet": {
        name: "Hollowed Bullet",
        pos: "d8",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "150 action points",
        range: "50ft range",
        cast: "1.5s cast",
        cooldown: "no cooldown",
        blue: "Execute",
        description: talentText`Deals 30% RAP + 283 physical damage to target enemy and applies 1 stack of Reduced Strength, Reduced Ballistic Skill, Reduced Intelligence, and Reduced Toughness.`,
      },
      "Splintering Hollow": {
        name: "Splintering Hollow",
        pos: "e8",
        icon: icons["tac_spec_5"],
        maxRank: 1,
        reqPoints: 35,
        prereq: "Hollowed Bullet",
        arrows: [{ dir: "right", from: "d8", to: "e8" }],
        type: "career tactic",
        description: talentText`Hollowed Bullet will now hit up to 9 other enemies within 20ft of the target for 33% damage as well as applying stacks.`,
      },
      "Rapid Reload": {
        name: "Rapid Reload",
        pos: "d9",
        icon: icons["noicon"],
        maxRank: 1,
        reqPoints: 40,
        prereq: "Hollowed Bullet",
        arrows: [{ dir: "down", from: "d8", to: "d9" }],
        type: "ability",
        cost: "150 action points",
        range: "self",
        cast: "instant cast",
        cooldown: "45s cooldown",
        blue: "Execute/nOff the GCD",
        description: talentText`For the next 4 seconds, Hollowed Bullet will have 100% action point cost reduction.`,
      },
      "Smash": {
        name: "Smash",
        pos: "e8",
        icon: icons["abi_smash"],
        maxRank: 1,
        reqPoints: 35,
        type: "skill",
        cost: "60 action points",
        range: "7.5ft range",
        cast: "instant cast",
        cooldown: "20s cooldown",
        description: talentText`Applies Knocked-down and Unstoppable to target enemy.\n\nKnocked-down prevents all control of your character for 3 seconds. Unstoppable prevents application of Knocked-down and Staggered for 30 seconds.`,
      },
      "Righteous Steel": {
        name: "Righteous Steel",
        pos: "f8",
        icon: icons["tac_spec_7"],
        maxRank: 1,
        reqPoints: 35,
        prereq: "Smash",
        arrows: [{ dir: "right", from: "e8", to: "f8" }],
        type: "career tactic",
        description: talentText`Bind Weapon, Silence Heretic, and Smash will now also deal 22% MAP + 160 physical damage to the target and generate 1 Combo Point on them.`,
      },
      "Dragon Gun": {
        name: "Dragon Gun",
        pos: "f9",
        icon: icons["abi_dragongun"],
        maxRank: 1,
        reqPoints: 40,
        type: "skill",
        cost: "150 action points",
        range: "self",
        cast: "2.0s cast",
        cooldown: "30s cooldown",
        blue: "Castable while moving",
        description: talentText`Applies an 8 second duration cripple and 5 Combo Points to up to 9 enemies 30ft in front of you. Each second the target and up to 9 enemies within 20ft of them will take 10% RAP + 100 elemental damage.`,
      },
    },
  },
};
